...
INSTRUMENT_COVERED_MANUAL = {
    ...
    # Степанова Ирина, 13.08 — депозит (списание 20 000₽ 13.08) + 2×сертификат «2000₽ Бонус» (по 221.49₽ погашены 13.08) = 20 442,98₽.
    # Услуги в записи по факту в порядке: [0]=Ботулинотерапия, [1]=Консультация (0₽, не в счёт), [2]=МОНАКО — исправлено
    # 2026-09-01, старые ключи (1,2) были заведены до того, как раскрылся реальный порядок услуг и никогда не применялись
    # (индекс 1 = бесплатная консультация). Полностью покрывает МОНАКО (14 678₽), остаток пула частично Ботулинотерапию —
    # 899,16₽ так и остаются необъяснённой скидкой.
    (1899256635, 2): 14678,     # МОНАКО
    (1899256635, 0): 5764.98,   # Ботулинотерапия, частично
}
