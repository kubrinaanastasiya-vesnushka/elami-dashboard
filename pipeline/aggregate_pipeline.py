...
INSTRUMENT_COVERED_MANUAL = {
    ...
    # 2026-09-14: индексы этих двух записей съехали между прогонами API (Тринити теперь si=0,
    # было si=1) — тот же класс нестабильности, что нашли на Степановой Ирине и Сулейкиной.
    (1816480947, 0): 3500,     # Аминева Эльвира, Тринити 01.08 — абонемент «Тринити 12 процедур» (карта 84 в логе, использование+перерасчёт 3150+350)
    (1816481442, 0): 3500,     # Аминева Эльвира, Тринити 03.08 — абонемент «Тринити 12 процедур» (карта 84 в логе, 03.08 20:43)
    ...
    (1819880520, 0): 1002,     # Журавлева Татьяна, ЛЭ Стандарт 16.08 — депозит, списание 1300₽ 16.08 (точное совпадение с суммой обеих строк 1002+298)
    (1819880520, 1): 298,      # Журавлева Татьяна, ЛЭ Верхняя губа 16.08 — тот же депозит
    (1897496529, 0): 3500,     # Зарипова Аида, Ручной массаж 60 мин 16.08 — абонемент «Ручной массаж 60 мин 5 процедур», погашен 16.08 (сумма списания 3150 + перерасчёт, тот же паттерн что и Тринити выше)
    (1899794628, 0): 3700,     # Амирова Айсылу, ЛЭ Премиум 16.08 — депозит 1700₽ (16.08) + сертификат 2000₽ (Бонус, погашен 16.08) = 3700 точное совпадение
    (1868204406, 0): 205.71,   # Топыло Екатерина, Плазмотерапия/PRP 13.08 — сертификат №2205-3, списание 205.71₽ (подтверждено скрином чека YClients от Насти)
    (1868204406, 1): 594.29,   # Топыло Екатерина, Контурная пластика губ 13.08 — тот же сертификат №2205-3, вторая часть списания 594.29₽ (один сертификат разбит на 2 позиции чека)
    (1833450216, 0): 1000,     # Садриева Юлия, Чистка+пилинг 22.08 — сертификат 1000₽, погашен 22.08 — частично покрывает гэп 2000₽, остаток 1000₽ так и остаётся скидкой
    ...
    # 2026-09-14: индексы съехали ещё раз (см. комментарий выше про Тринити) — МОНАКО теперь
    # si=1, Ботулинотерапия si=2.
    (1899256635, 1): 14677.68,  # МОНАКО — весь разрыв
    (1899256635, 2): 6664.46,   # Ботулинотерапия — весь разрыв
    ...
    # 20 000 + 2 сертификата «2000руб (Бонус)» №99+№100 (по 2000 каждый) = 25 900. Первая
    # версия покрывала только депозит (20 000), забыла 2 сертификата (4 000) — теперь весь
    # разрыв закрыт, реальной скидки нет вообще.
    # 2026-09-14, второй фикс: services-массив этой записи переставился местами между
    # прогонами fetch_pipeline.py (Консультация теперь si=0, Ботулинотерапия si=1 — было
    # наоборот при первом фиксе) — тот же класс бага, что раньше словили на Степановой Ирине.
    # Индекс si НЕ стабилен между прогонами API, поэтому такие оверрайды хрупкие в принципе.
    (1905825039, 1): 24000,   # весь разрыв: депозит 20 000 + сертификаты №99+№100 (2000+2000)
    ...
    # 2026-09-14: services-массив этой записи переставился местами между прогонами API —
    # индексы указаны по актуальному порядку на момент последнего deploy.sh.
    (1950445173, 1): 157.23,  # Бойко Снежана, Контурная пластика губ 05.09 — тот же депозит-списание 500₽
    (1950445173, 0): 314.47,  # Бойко Снежана, Армирование/векторный лифтинг 05.09 — тот же депозит-списание 500₽
}
