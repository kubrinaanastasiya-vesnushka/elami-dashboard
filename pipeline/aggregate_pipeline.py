...
#
# 2026-09-14: keyed by (record_id, service TITLE), not index — found (repeatedly, on
# Степанова Ирина, then Сулейкина Наталья, then the Аминева Эльвира Тринити records) that
# YClients does NOT guarantee stable ordering of a record's services array across separate
# API fetches. An index-based key silently starts pointing at the wrong line the next time
# fetch_pipeline.py reruns (every deploy). Title is stable; resolved to the current run's
# actual index right below, after this dict.
INSTRUMENT_COVERED_MANUAL_BY_TITLE = {
    (1846601913, "Ботулинотерапия ед."): 20527,  # Денисенко Наталья, 28.07 — сертификаты (см. certificates CSV, 45 000₽ на этот телефон куплено 16.07.2026)
    (1846601913, "Биоревитализация"): 10473,     # Денисенко Наталья, 28.07 — сертификаты
    (1866933444, "Тредлифтинг/мезонити/мононити"): 18000,  # Солодовникова Светлана, 27.07 — депозит 15 000₽ (26.07, с рассылки) + сертификат 3 000₽ (сумма списания 27.07, точное совпадение)
    (1810795734, "ЛЭ ВИП"): 5000,   # Куцарь Дарья, 04.07 — сертификат 5 000₽ (продан 21.06, погашен 04.07, точное совпадение)
    (1845023856, "Ботулинотерапия ед."): 2000,   # Сатыбалдыева Лилия, 15.07 — сертификат (сумма списания 2 000₽ 15.07, точное совпадение)
    ...
    (1850420241, "Комбо-лифтинг (массаж лица+RF лифтинг+гальванический ток)"): 3500,  # Останина Наталья, 07.08 — списание с личного счёта 3500₽ (лог, 07.08)
    (1856621310, "Контурная пластика губ"): 5869.64,  # Пивовар Ксения, 03.08 — списание с личного счёта (лог, 03.08, точное совпадение до копейки)
    (1856621310, "МОНАКО (биоревитализация)"): 11430.36, # Пивовар Ксения, 03.08 — списание с личного счёта (лог, 03.08, точное совпадение до копейки)
    (1873014504, "Контурная пластика губ"): 10000,    # Брежнева Дина, 01.08 — списание с сертификата 10000₽ (лог, 01.08)
    (1816480947, "Массаж Тринити терапия"): 3500,     # Аминева Эльвира, 01.08 — абонемент «Тринити 12 процедур» (карта 84 в логе, использование+перерасчёт 3150+350)
    (1816481442, "Массаж Тринити терапия"): 3500,     # Аминева Эльвира, 03.08 — абонемент «Тринити 12 процедур» (карта 84 в логе, 03.08 20:43)
    (1874730804, "ЛЭ ПРЕМИУМ"): 4600,     # Шестакова Мария, 08.08 — абонемент «Пакет Премиум 5 процедур» (лог, использование+перерасчёт 3485+1115=4600, полностью)
    # 2026-08-25: Nastya sent updated certificates/abonements/deposit-operations exports
    # (01.01.2026-25.08.2026) — matched by phone against the "без категории" August list.
    (1819880520, "ЛЭ СТАНДАРТ"): 1002,     # Журавлева Татьяна, 16.08 — депозит, списание 1300₽ 16.08 (точное совпадение с суммой обеих строк 1002+298)
    (1819880520, "ЛЭ Верхняя губа"): 298,      # Журавлева Татьяна, 16.08 — тот же депозит
    (1897496529, "Ручной массаж всего тела оздоровительный 60 мин"): 3500,     # Зарипова Аида, 16.08 — абонемент «Ручной массаж 60 мин 5 процедур», погашен 16.08 (сумма списания 3150 + перерасчёт, тот же паттерн что и Тринити выше)
    (1899794628, "ЛЭ ПРЕМИУМ"): 3700,     # Амирова Айсылу, 16.08 — депозит 1700₽ (16.08) + сертификат 2000₽ (Бонус, погашен 16.08) = 3700 точное совпадение
    (1868204406, "Плазмотерапия/PRP терапия"): 205.71,   # Топыло Екатерина, 13.08 — сертификат №2205-3, списание 205.71₽ (подтверждено скрином чека YClients от Насти)
    (1868204406, "Контурная пластика губ"): 594.29,   # Топыло Екатерина, 13.08 — тот же сертификат №2205-3, вторая часть списания 594.29₽ (один сертификат разбит на 2 позиции чека)
    (1833450216, "Чистка+пилинг"): 1000,     # Садриева Юлия, 22.08 — сертификат 1000₽, погашен 22.08 — частично покрывает гэп 2000₽, остаток 1000₽ так и остаётся скидкой
    # Степанова Ирина, 13.08 — Настя подтвердила скриншотом чека YClients (2026-09-01):
    # «Оплачено полностью» — 30 250₽ разложены на наличные 6 250 + депозит 20 000 + 4 сертификата на 4 000 (все счета в
    # одну кассу зашли восемью отдельными строками из-за нескольких услуг/сертификатов в одном чеке). Остаток 899₽,
    # который ledger не мог объяснить депозитом/сертификатами — это и есть та самая наличная часть, скидки тут нет вообще.
    # Полностью закрываю обе строки услуг.
    (1899256635, "МОНАКО (биоревитализация)"): 14677.68,  # весь разрыв
    (1899256635, "Ботулинотерапия ед."): 6664.46,   # весь разрыв
    ...
    # Сулейкина Наталья, Ботулинотерапия 09.09 — Настя подтвердила скриншотом чека YClients
    # (2026-09-14): «Оплачено полностью» — 25 900₽ разложены на карту/перевод 1 900 + депозит
    # 20 000 + 2 сертификата «2000руб (Бонус)» №99+№100 (по 2000 каждый) = 25 900. Весь разрыв закрыт,
    # реальной скидки нет вообще.
    (1905825039, "Ботулинотерапия ед."): 24000,
    (1911293964, "ЛЭ ПРЕМИУМ"): 600,     # Тимохова Лариса, 12.09 — депозит, списание 600₽ 12.09 18:24
    ...
    # Почетова Вера, 16.09 — Настя подтвердила скриншотом чека YClients (2026-09-22):
    # «Оплачено полностью» 15 400₽ = карта 5 400 + сертификат «10000руб — №87» дважды
    # (7 727,27 + 2 272,73 = 10 000). Сертификата нет в экспорте по её номеру (видимо, куплен
    ...
    # Астахова Роза, 05.09, 11:00 — Настя подтвердила (2026-09-22): пробный массаж, тут же на
    # месте купила абонемент «Массаж антицеллюлитный 40мин 10 сеансов №106» (тот же день,
    # 12:11, 29 750₽) — первый визит зачтён как первая сессия абонемента задним числом,
    # ledger не мог увидеть это сам (визит хронологически раньше покупки).
    (1937913741, "Антицеллюлитный массаж 40мин"): 3500,
}
_all_records_by_id = {r["id"]: r for _raw in MONTHLY_RAW.values() for r in _raw["records"]}
INSTRUMENT_COVERED_MANUAL = {}
for (_mrid, _mtitle), _mamt in INSTRUMENT_COVERED_MANUAL_BY_TITLE.items():
    _mrec = _all_records_by_id.get(_mrid)
    if not _mrec:
        continue
    for _msi, _ms in enumerate(_mrec.get("services", [])):
...
def month_metrics(ym):
    ...
    return {
        ...
        # Nastya (2026-09-14): % теперь считается от базы услуги+товары (с учётом скидки),
        # раз сами скидки с этой даты тоже включают товарные — раньше знаменатель был только
        # по услугам, что занижало % после добавления товарных скидок в числитель.
